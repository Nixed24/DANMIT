...
#Is the VMF in Hammer++ format? Set to "False" if you're not sure.
...
def get_fourth_vertex(plane_points):
    ...
    if const_axis == -1:
        raise ValueError("No constant axis found in a side plane. Are all of your brushes cuboidal?")
    #We know the plane that the points form will have normal vector in the direction of const_axis
    ...
    for a in ax_nt:
        fourth_point[a] = point_array[a][dupe_table[a] == 1]
    return fourth_point
...
def gen_normal_map_image(normal_map):
    ...
    pixels = img.load()
    for i in range(img.size[1]):    # For every column
...
for y_index, current_y in enumerate(y_centres):
    ...
    for x_index, current_x in enumerate(x_centres):
        ...
        ##-- Generating ordered dispinfo dict, hence generating displacement field dict --##
        
        #-- (well, 2 lists, but logically a dict) --##
...
normal_map = gen_normal_map(grand_displacement_field)
